...
export class AgentCacheService {
  ...
  private static async getAgentListings(agentName: string): Promise<Property[]> {
    try {
      ...
      // Create name variations for better matching (full names only, no partials)
      const agentNameLower = agentName.toLowerCase();
      const nameVariations = [
        agentNameLower,
        agentNameLower.replace('chris ', 'christopher '), // Only replace when followed by space
        agentNameLower.replace('christopher ', 'chris '),
        agentNameLower.replace(' steve', ' stephen'), // Only replace when preceded by space
        agentNameLower.replace(' stephen', ' steve'),
        // Handle Lynda Werner / Lynda Sanchez-Werner
        agentNameLower.replace('lynda werner', 'lynda sanchez-werner'),
        agentNameLower.replace('lynda sanchez-werner', 'lynda werner'),
      ];
      ...
      for (const cacheEntry of properties) {
        try {
          ...
          if (property.ListAgentFullName) {
            ...
            // First try exact match or full name variations
            let nameMatch = nameVariations.some(variation => 
              propertyAgentName === variation || variation === propertyAgentName
            );
            
            // If no exact match, try matching first and last name separately
            // This handles cases like "Lynda Werner" matching "Lynda Sanchez-Werner"
            if (!nameMatch && agentNameLower.includes(' ')) {
              const agentParts = agentNameLower.split(/[\s-]+/);
              const propertyParts = propertyAgentName.split(/[\s-]+/);
              
              // Require first name to match exactly
              const firstNameMatch = agentParts[0] === propertyParts[0];
              
              // For last name, check if any part contains the agent's last name
              // This handles hyphenated names like "Sanchez-Werner" matching "Werner"
              const lastNameMatch = agentParts.length > 1 && propertyParts.length > 1 && 
                (agentParts[agentParts.length - 1] === propertyParts[propertyParts.length - 1] ||
                 propertyParts.some((part: string) => part === agentParts[agentParts.length - 1]) ||
                 agentParts.some((part: string) => part === propertyParts[propertyParts.length - 1]));
              
              // Only match if BOTH first and last name match
